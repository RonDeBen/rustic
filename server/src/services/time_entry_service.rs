...
pub async fn switch_to_timer<'a, A>(conn: A, id: i32) -> Result<TimeEntry, sqlx::Error>
where
    A: Acquire<'a, Database = Postgres>,
{
    ...
    let running_timers = fetch_all_running_timers(&mut *pool).await?;
    for timer in running_timers {
        pause_timer(&mut *pool, timer).await?;
    }

    // start new timer
    let start_time: NaiveDateTime = Utc::now().naive_utc();
    play_time_entry(&mut *pool, id, start_time).await
}

pub async fn pause_timer<'a, A>(conn: A, entry: TimeEntry) -> Result<TimeEntry, sqlx::Error>
where
    A: Acquire<'a, Database = Postgres>,
{
    ...
    pause_time_entry(conn, entry.id, elapsed_time).await
}
